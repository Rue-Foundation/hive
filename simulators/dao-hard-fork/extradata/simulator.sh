...
proID=`curl -sf -X POST $HIVE_SIMULATOR/nodes?HIVE_FORK_DAO_VOTE=1\&HIVE_MINER=0x0000000000000000000000000000000000000001`
proIP=`curl -sf $HIVE_SIMULATOR/nodes/$proID`
echo "Started pro-fork node $proID at $proIP"

conID=`curl -sf -X POST $HIVE_SIMULATOR/nodes?HIVE_FORK_DAO_VOTE=0\&HIVE_MINER=0x0000000000000000000000000000000000000001\&HIVE_MINER_EXTRA=dao-hard-fork`
...
proLatest=0
while [ true ]; do
	proLatest=`curl -sf -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' $proIP:8545 | jq '.result' | tr -d '"'`
...
conLatest=0
while [ true ]; do
	conLatest=`curl -sf -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' $conIP:8545 | jq '.result' | tr -d '"'`
...
for block in `seq 1 $proLatest`; do
	extra=`curl -sf -X POST -H "Content-Type: application/json" --data "{\"jsonrpc\":\"2.0\",\"method\":\"eth_getBlockByNumber\",\"params\":[$block, false],\"id\":3}" $proIP:8545 | jq '.result.extraData' | tr -d '"'`
...
for block in `seq 1 $conLatest`; do
	extra=`curl -sf -X POST -H "Content-Type: application/json" --data "{\"jsonrpc\":\"2.0\",\"method\":\"eth_getBlockByNumber\",\"params\":[$block, false],\"id\":3}" $conIP:8545 | jq '.result.extraData' | tr -d '"'`
